...
##########
# LTT summarizing functions
##########

# Function for extracting information out of LTT plot
netDropSummary <- function(ltt,t.min,t.max) {
  t <- ltt$x
  n <- ltt$y
  ...
  peak.end <- min(which(n == max(n)))
  peak.start <- peak.end + 1
  peak.mid <- (t[peak.start] + t[peak.end])/2
  peak.n <- n[peak.end]
  
  trough.end <- min(which(n == min(n)))
  trough.start <- trough.end + 1
  trough.mid <- (t[trough.start] + t[trough.end])/2
  trough.n <- n[trough.end]
  
  dn_dt <- (trough.n - peak.n)/(peak.mid - trough.mid)
  
  res <- c((trough.n - peak.n),dn_dt,peak.mid,trough.mid)
  names(res) <- c("delta","slope","time.max.n","time.min.n")
  return(res)
}


# Function for extracting information out of LTT plot
totalDecreaseSummary <- function(ltt,t.min,t.max,normalize=FALSE) {
  # recover()
  t <- ltt$x
  n <- ltt$y
  ...
  # Number of times the LTT curve decreases == number of samples
  dn <- sum(n[-1] < n[-length(n)])
...
# Which interval has the most fossils?
maxDecrease <- function(ltt,grid.times) {
  all_deltas <- sapply(1:(length(grid.times)-1),function(i){
    totalDecreaseSummary(ltt,grid.times[i],grid.times[i+1])
  })
  which.max(all_deltas)
}
